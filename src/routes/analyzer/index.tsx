...
export const useInitialAnalysis = routeAction$(async ({ url }, { fail }) => {
  try {
...
export default component$(() => {
  ...
  // Handle URL parameter and initial analysis
  useTask$(({ track }) => {
    ...
    if (currentUrl && isBrowser) {
      ...
      initialAnalysis.submit({ url: currentUrl });
    }
  });

  // Handle analysis results and loading state
  useTask$(({ track }) => {
    const analysis = track(() => initialAnalysis.value);
    const pending = track(() => initialAnalysis.isRunning);

    // Update loading state based on action status
    ...
    if (analysis && !analysis.failed) {
      const data = analysis as { success: true; data: RobotsAnalysisResult };
      console.log("Setting result:", data.data);
      result.value = data.data;
      error.value = "";
    } else if (analysis?.failed) {
      const errorData = analysis as { error: string; failed: true };
      error.value = errorData.error || "Failed to analyze robots.txt";
      result.value = null;
    }
  });
  ...
  const analyzeRobotsTxtFile = $(async (inputUrl: string) => {
    ...
    try {
      // Use the routeAction instead of direct API call
      await initialAnalysis.submit({ url: inputUrl });
  ...
  const showResults = result.value !== null;
  const showLoading = isLoading.value;
  const showInitial = !result.value && !isLoading.value;
